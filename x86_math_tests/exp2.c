...
void EXP2_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	EXP2_spTest(psFuncName,ui32Size,pfSrcBuffer);

	EXP2_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void EXP2_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = exp2f(pfBuffer[ui32Idx]);
		}
...
void EXP2_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = exp2(pdBuffer[ui32Idx]);
		}
