...
void LOG2_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	LOG2_spTest(psFuncName,ui32Size,pfSrcBuffer);

	LOG2_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void LOG2_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = log2f(pfBuffer[ui32Idx]);
		}
...
void LOG2_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = log2(pdBuffer[ui32Idx]);
		}
