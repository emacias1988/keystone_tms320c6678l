...
void POW_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	POW_spTest(psFuncName,ui32Size,pfSrcBuffer);

	POW_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void POW_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = powf(pfBuffer[ui32Idx],(float)2.3456789);
		}
...
void POW_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = pow(pfBuffer[ui32Idx],(double)2.3456789);
		}
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = pow(pfBuffer[ui32Idx],(double)2.345632324789);
		}
