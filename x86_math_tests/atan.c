...
void ATAN_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	ATAN_spTest(psFuncName,ui32Size,pfSrcBuffer);

	ATAN_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void ATAN_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = atanf(pfBuffer[ui32Idx]);
		}
...
void ATAN_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = atan(pdBuffer[ui32Idx]);
		}
