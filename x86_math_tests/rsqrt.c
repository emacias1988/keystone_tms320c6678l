...
void RSQRT_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	RSQRT_spTest(psFuncName,ui32Size,pfSrcBuffer);

	RSQRT_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void RSQRT_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = 1/sqrtf(pfBuffer[ui32Idx]);
		}
...
void RSQRT_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = 1/sqrt(pdBuffer[ui32Idx]);
		}
