...
void EXP10_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	EXP10_spTest(psFuncName,ui32Size,pfSrcBuffer);

	EXP10_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void EXP10_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = pfBuffer[ui32Idx] * (float) logf(10);
		}
...
void EXP10_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = pdBuffer[ui32Idx] * (double) log(10);
		}
