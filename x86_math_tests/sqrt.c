...
void SQRT_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	SQRT_spTest(psFuncName,ui32Size,pfSrcBuffer);

	SQRT_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void SQRT_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = sqrtf(pfBuffer[ui32Idx]);
		}
...
void SQRT_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = sqrt(pdBuffer[ui32Idx]);
		}
