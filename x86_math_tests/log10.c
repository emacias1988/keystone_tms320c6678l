...
void LOG10_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	LOG10_spTest(psFuncName,ui32Size,pfSrcBuffer);

	LOG10_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void LOG10_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = log10f(pfBuffer[ui32Idx]);
		}
...
void LOG10_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = log10(pdBuffer[ui32Idx]);
		}
