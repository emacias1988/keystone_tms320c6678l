...
void RECIP_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	RECIP_spTest(psFuncName,ui32Size,pfSrcBuffer);

	RECIP_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void RECIP_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = (float) 1/pfBuffer[ui32Idx];
		}
...
void RECIP_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = (double) 1/pfBuffer[ui32Idx];
		}
