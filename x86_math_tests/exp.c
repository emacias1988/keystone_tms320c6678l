...
void EXP_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	EXP_spTest(psFuncName,ui32Size,pfSrcBuffer);

	EXP_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void EXP_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = expf(pfBuffer[ui32Idx]);
		}
...
void EXP_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = exp(pdBuffer[ui32Idx]);
		}
