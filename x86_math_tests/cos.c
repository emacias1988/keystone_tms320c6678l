...
void COS_runTests(char * psFuncName, uint32_t ui32Size, float * pfSrcBuffer, float * pfDestBuffer)
{
	...
	COS_spTest(psFuncName,ui32Size,pfSrcBuffer);

	COS_dpTest(psFuncName,ui32Size,pfSrcBuffer);
}
...
void COS_spTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pfDestBuffer[ui32Idx] = cosf(pfBuffer[ui32Idx]);
		}
...
void COS_dpTest(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	...
	while(ui32TotalSize)
	{
		...
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			g_pdDestBuffer[ui32Idx] = cos(pdBuffer[ui32Idx]);
		}
