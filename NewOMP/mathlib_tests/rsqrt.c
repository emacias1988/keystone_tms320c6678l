...
void RSQRT_runTests(char * psFuncName, uint32_t ui32Size, float * pfBuffer)
{
	RSQRT_spTest(psFuncName,ui32Size,pfBuffer);

//	RSQRT_dpTest(psFuncName,ui32Size,pfBuffer);
}
...
#pragma omp for
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			pfBuffer[ui32Idx] = rsqrtsp(pfBuffer[ui32Idx]);
//			coreid = omp_get_thread_num(); 		  // Get master core
//			printf ("Core: %d Result[%d] of rsqrtsp is %f\n",coreid,ui32Idx,(float)pfBuffer[ui32Idx]);
...
//	{
//
//		printf ("Result[%d] of rsqrtsp is %f\n",ui32Idx,(float)pfBuffer[ui32Idx]);
//
//	}
...
#pragma omp for
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			pfBuffer[ui32Idx] = rsqrtsp_i(pfBuffer[ui32Idx]);
		//	printf ("Result of rsqrtsp_i is %f\n",pfBuffer[ui32Idx]);
...
#pragma omp for
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			pdBuffer[ui32Idx] = rsqrtdp(pdBuffer[ui32Idx]);
...
#pragma omp for
		for(ui32Idx=0;ui32Idx<ui32IdxCount;ui32Idx++)
		{
			pdBuffer[ui32Idx] = rsqrtdp_i(pdBuffer[ui32Idx]);
